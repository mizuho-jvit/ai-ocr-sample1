...
- 判断材料として、(a) 本番反映前のステージング確認（ワンクッション）が要るか、(b) リリース後に過去バージョンだけを緊急修正する場面があり得るかをユーザーへ確認した。(a)は「デモ用の単一環境で常に最新をそのまま見せる」ため不要、(b)は「あるかもしれない」との回答だった。
- タグのみで管理する方式（`main`1本＋リリース時にタグを打つだけ）も検討したが、(b)への対応にはタグの時点から都度即興でブランチを作る必要が生じること、また担当者の交代・複数人体制になる可能性を踏まえ「今どの状態が本番か」を常設のブランチ名で明示できる方が引き継ぎに強いことから、`release`ブランチの新設を採用した。
- 実装(`git tag`/`git branch`の実操作、GitHub Actionsのデプロイジョブ追加)はまだ行っていない。`knowledge/wiki/requirements/decisions.md`（決定#53）・`operations.md`（デプロイ運用セクション）に記録した。
...
- **[P1] 最後の有効なadminを無効化・staffへ降格できる欠陥を修正した（決定#44）。** `StaffService.update`が、自分自身を含む任意の有効なadminへ`isActive: false`や`role: "staff"`を設定できてしまい、シード管理者1名の状態で実行すると`/api/staff`へアクセスできる利用者が0人になりD1を直接修復する以外に復旧手段が無くなる欠陥だった。この更新で対象行が「有効なadmin」でなくなる場合に限り、他に有効なadminが存在するかをDB側で検証する条件付きUPDATE(`src/worker/db/atomic-writes.ts`の`updateStaffUserGuarded`)へ回し、存在しなければ`409 INVALID_TRANSITION`(既存コードを再利用)で拒否するようにした。並行更新でも不変条件が保たれることを、ほぼ同時に2人の有効なadminが互いを降格し合うテストで確認した(このテストは「読み取ってから判定する」ナイーブな実装に一時的に戻すと確実に失敗することも確認済み)。api.md #26のステータスコードに`409`を追加した。
- **[P1] 無効化した職員の既存セッションが、再有効化すると復活する欠陥を修正した（決定#45）。** 無効化(`isActive: false`)時に`staff_users.isActive`だけを更新し、対象職員の`sessions`行を削除していなかったため、無効中は`resolveSession()`が拒否するもののセッション行自体は12時間の有効期限まで残り、再有効化すると無効化前に発行された古いCookieが有効期限内であれば再び使えてしまう欠陥だった。上記の`updateStaffUserGuarded`へ`deleteSessions`オプションを追加し、`isActive`をtrue→falseへ変える更新に限り、対象職員の`sessions`を`D1Database.batch`(単一トランザクション)で全削除するようにした(再有効化・無効化以外の更新では削除しない)。テストは「ログイン済み職員を無効化するとセッションが削除される」「再有効化しても古いCookieでは認証できない」「再有効化後に新しくログインすれば利用できる」「無効化以外の更新ではセッションを消さない」の4件で、いずれも修正前に戻すと失敗することを確認済み。api.mdの判断#10・スタッフ管理節を更新した。
- **[P2] 同一メールの並行登録で一方が500になる欠陥を修正した（決定#46）。** `StaffService.create`のメール重複確認(決定#43)はSELECT→INSERTの事前検証であり、2件の登録リクエストが並行すると両方が「未登録」と判定してINSERTへ進み、一方が`staff_users_tenant_email_unique`違反の生の例外(D1/drizzleの`DrizzleQueryError`)をそのまま投げてしまい、API契約上`422`であるべき重複登録が競合時だけ`500`になっていた。`insert`をtry/catchし、`DrizzleQueryError.cause.message`に`"UNIQUE constraint failed"`と`"staff_users.email"`が含まれる場合に限り`422 VALIDATION_ERROR`へ変換するようにした(D1/drizzleは制約違反を専用の型で公開しないため、実機で実際に投げられる例外の形を確認したうえで文字列判定を採用)。事前SELECT(決定#43・早期エラー表示用)はそのまま残す。同一メールで2件を並行作成すると1件だけ成功しもう1件が422になることをテストで確認した(このテストは修正前に戻すと`DrizzleQueryError`がそのまま伝播して確実に失敗することも確認済み)。
- **[P2] メール形式・パスワード強度をサーバー側で検証していなかった欠陥を修正した（決定#47）。** `routes/staff.ts`の`parseCreateStaffRequest`／`parseUpdateStaffRequest`は、メールが空でないこと・パスワードが1文字以上であることしか検証しておらず、`invalid`のようなメールや1文字のパスワードでも登録できていた。要件（F-1・F-7）に基準が無いため実装時判断として、メールは簡易パターン`^[^\s@]+@[^\s@]+\.[^\s@]+$`(前後・埋め込み空白を含む値は自動的に拒否。トリム等の正規化はしない)かつ最大254文字(RFC 5321の実務上限)、パスワードは最小12文字・最大50文字(ユーザー指定)を採用し、`POST`（メール・パスワード双方）・`PATCH`（パスワードのみ。メールは更新対象外）の境界で検証するようにした。保存する`passwordHash`はハッシュ・salt固定長のBase64であり元のパスワード長を保存しないため、最大長を50文字にしてもDB格納値の長さには影響しない。api.mdのスタッフ管理節へ検証基準を明記した。
- テストを`test/worker/services/staff-service.test.ts`へ10件(P1欠陥1に5件、P1欠陥2に4件、P2欠陥1に1件)、`test/worker/routes/staff.test.ts`へ11件(`INVALID_TRANSITION`→409のマッピング1件、メール形式・空白・長さの拒否6件、パスワード長の拒否3件(POST 2件・PATCH追加1件)、境界値の受理2件)追加した。いずれも修正前に戻すと失敗することを確認済み。`corepack pnpm test`（55ファイル・564テスト）/ `lint` / `tsc -b` / `build`で確認した。

### スタッフ管理を実装した（Task 012）

- **`GET /api/staff`（一覧・api.md #24）・`POST /api/staff`（登録・#25）・`PATCH /api/staff/:id`（編集・無効化・#26）をすべて実装した。** ルート（`src/worker/routes/staff.ts`）は検証とセッション解決のみを行い、業務ロジックは新設の`StaffService`（`src/worker/services/staff-service.ts`）へ委ねた（`member-service.ts`と同じ責務分離）。
- **F-7-2（staffロールは全スタッフAPIで403）は、既存の`middleware/auth.ts`の`adminGuard()`を`routes.use("*", adminGuard())`でルーター全体へ一括適用して満たした。** `adminGuard`/`requireAdmin`自体はTask 003で実装済みだったが、実際のAPIルートへ適用するのは本タスクが初めて。
- **削除APIは設けず、無効化は`PATCH`の`isActive: false`のみで表す（Implementation Notes）。** 「無効化職員は以後ログインできない」（Test Strategy）は、既存の`services/auth.ts`のログイン・セッション解決が既に`staff.isActive`を検証しているため新規実装は不要と判明し、`staff-service.test.ts`に`update(isActive:false)`後の`login`が`INVALID_CREDENTIALS`になる統合テストを追加して確認した。
- **メールアドレスの重複（`staff_users_tenant_email_unique`）を422で返す実装時判断。** D1の一意制約違反を捕捉して特定のエラーコードへ変換する前例がリポジトリに無かったため、`create`側で`findOne`により重複を事前検証してから`insert`する方式にした（決定は伴わない実装時判断）。
- **SPA側は`src/react-app/{api/staff.ts,pages/staff-page.tsx}`を新規実装した。** `screen-list.md`は「スタッフ管理」の1画面のみを定めるため、会員一覧のトグル式登録フォームと同じ考え方で、一覧内のインライン行編集（氏名・ロール・有効/無効・新パスワードの各フィールドを行内で編集）で登録・編集・無効化を完結させ、独立した登録・詳細画面は作らなかった。`AppShell`のナビメニューから「スタッフ管理」（admin限定）を有効化した。
- Worker側は`test/worker/{routes/staff,services/staff-service}.test.ts`を、SPA側は`test/react-app/{api/staff,pages/staff-page}.test.tsx`を追加し、`app-shell.test.tsx`にadminがナビから遷移できることを確認するテストを1件足した。`corepack pnpm test`（55ファイル・540テスト）/ `lint` / `tsc -b` / `build` で確認した。ローカルのBasic認証情報（`.dev.vars`）を読まない方針のため、ブラウザでの実機確認は今回省略した。
...
- Task 022で新設した`AppStatusBadge`の色対応表（`APP_STATUS_BADGE_COLOR`）をエクスポートして再利用し、`application-detail-page.tsx`のステータス変更ボタンへ同じ配色をインラインstyleで適用した。塗りつぶしはプロトタイプでは「現在のステータス」を示す用途だが、実装では対象外（=押せる）の遷移先しかボタンを出さないため、常にアウトラインでよい。
- `test/react-app/pages/application-detail-page.test.tsx`に、承認=緑・差戻し=朱色になることを確認するテストを追加した。`corepack pnpm test`（42ファイル・385テスト）/ `lint` / `tsc -b` / `build` で確認した。
