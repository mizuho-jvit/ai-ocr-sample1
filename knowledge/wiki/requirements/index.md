...
AI-OCR 帳票読取・一次審査システム（営業デモ用）の要件。正典化元は要件定義書 v1.11（`knowledge/ref/doc/claude_code_要件定義_v1.11.md`）で、**確定済み・未確定事項なし**（唯一の例外は[検討中の論点（未確定）](./decisions.md#検討中の論点未確定)を参照）。OCR構成はMVP 1.0をGemini単体、MVP 1.1をDocument AI＋Geminiとする。
