...
flowchart TD
    ...
    WORD{"確認語が一致するか"}
    UNP["422"]
    K["① 削除対象の imageKey・snapshotToken を収集"]
    SNAP{"snapshotTokenがpreview時点と一致するか"}
    STALE["409<br/>preview後に対象が増減した(決定#51)"]
    D["② D1 のレコードを削除<br/>外部キーの依存順に 子テーブルから"]
    R["③ R2 のオブジェクトを削除<br/>①で確定したimageKeyの集合のみを1000件ごとに分割<br/>prefixの事後列挙はしない(決定#50)"]
...
| ③はprefixの事後列挙ではなく①で確定した集合だけを対象にする | F-9-12・[決定#50](../requirements/decisions.md)（②完了後・③実行前の間に別リクエストがR2へ保存した画像まで、事後列挙だと巻き込んで削除してしまう） |
| ①の直後に`snapshotToken`を検証し、不一致なら②以降へ進まず409で停止する | F-9-7・[決定#51](../requirements/decisions.md)（`preview`表示から実行までの間に申請・非seed会員が増減すると、確認画面で提示した件数より多く／少なく削除してしまう） |
